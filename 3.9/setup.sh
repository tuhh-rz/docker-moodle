...
echo "$(date +"%Y-%m-%d %T %s") Install Moodle if neccesary"
...
su -s /bin/bash -c "/usr/bin/php ${WEBSERVER_ROOT}${RELATIVE_PATH}admin/cli/install.php --non-interactive --agree-license --dataroot=$DATAROOT --chmod=$CHMOD --lang=$LANG --wwwroot=$WWWROOT --dbtype=$DBTYPE --dbhost=$DBHOST --dbname=$DBNAME --dbuser=$DBUSER --dbpass=$DBPASS --dbport=$DBPORT --prefix=$PREFIX --fullname='$FULLNAME' --shortname='$SHORTNAME' --summary='$SUMMARY' --adminuser=$ADMINUSER --adminpass=$ADMINPASS --adminemail=$ADMINEMAIL" www-data

echo "$(date +"%Y-%m-%d %T %s") Upgrade Moodle if neccesary"
su -s /bin/bash -c "/usr/bin/php ${WEBSERVER_ROOT}${RELATIVE_PATH}admin/cli/upgrade.php --non-interactive" www-data

echo "$(date +"%Y-%m-%d %T %s") Ensure cron.php"
echo "*/5 * * * * www-data /usr/bin/php ${WEBSERVER_ROOT}${RELATIVE_PATH}admin/cli/cron.php" >>/etc/crontab
...
chmod -w "${WEBSERVER_ROOT}${RELATIVE_PATH}"config.php
